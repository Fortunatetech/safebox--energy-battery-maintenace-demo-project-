...
# === Centered Main Title ===
st.markdown("""
    <h1 style='text-align: center;'>🔋 SAFEBOX ENERGY RECYCLING CHAMBER 035 </h1>
    <h3 style='text-align: center;'> MONITORING AND SORTING DASHBOARD </h3>
""", unsafe_allow_html=True)
...
if uploaded:
    ...
    # Convert timestamp to month labels
    window['month'] = window['timestamp'].dt.to_period('M').dt.to_timestamp()
    # Monthly time‑series charts
    t1,t2 = st.columns(2)
    t1.plotly_chart(px.line(window, x='month', y=['voltage_drop_V','current_draw_A'],
                             title='Voltage & Current (Monthly)'), use_container_width=True)
    t2.plotly_chart(px.line(window, x='month', y=['temp_delta','internal_resistance_ohm'],
                             title='Temp Δ & Resistance (Monthly)'), use_container_width=True)

    st.subheader("⏳ Charge Duration Over Months")
    fig_month = px.scatter(window, x='month', y='charge_duration_h',
