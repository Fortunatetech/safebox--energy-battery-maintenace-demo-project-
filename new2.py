...
# === Centered Main Title ===
st.markdown("""
    <h1 style='text-align: center;'>🔋 Battery Residual Life Monitoring Dashboard</h1>
...
if uploaded_file:
    ...
    # Add this block right after the KPI cards or Battery Life Gauge section

    # === Color-coded Visual Diagnosis Indicator ===
    st.subheader("🔍 Visual Diagnosis")

    # Define thresholds
    if selected_data['predicted_residual_life'] >= 600:
        diagnosis_color = "green"
        diagnosis_label = "Healthy"
    elif 300 <= selected_data['predicted_residual_life'] < 600:
        diagnosis_color = "orange"
        diagnosis_label = "Moderate"
    else:
        diagnosis_color = "red"
        diagnosis_label = "Critical"

    st.markdown(f"""
    <div style='text-align: center;'>
        <span style='background-color: {diagnosis_color}; color: white; padding: 0.5em 1em; border-radius: 10px; font-size: 1.5em;'>
            {diagnosis_label}
        </span>
    </div>
    """, unsafe_allow_html=True)
